...
FUNCTION convert_additional_night_viirs_netcdf_variables_to_gds2,i_filename,i_out_filename,$
             i_num_lons,$
             i_num_lats,$
             i_convert_to_kelvin, $
...
routine_name = "convert_additional_night_viirs_netcdf_variables_to_gds2";
msg_type = "";
msg = ""
i_data = "";

debug_module = 'convert_additional_night_viirs_netcdf_variables_to_gds2:';
debug_mode = 0
...
ok_to_write_sst3_flag      = 0;
ok_to_write_bias_sst3_flag = 0;
ok_to_write_stdv_sst3_flag = 0;
ok_to_write_qual_sst3_flag = 0;
...
; MODIS_L2P_ERROR_STEP 18: If cannot read optional additional night variable 'sst3' from night file, WARN sigevent thrown, keep going 
...
if (r_status NE SUCCESS) then begin
    ...
    donotcare = wrapper_ghrsst_notify_operator($
                    ...
                    msg,$
                    i_data);
    ok_to_write_sst3_flag = 0;  We won't be doing anything with the sst3 variable.
    ; Keep going
endif; end if ((r_status NE SUCCESS)

if (ok_to_write_sst3_flag) then begin
...
if (ok_to_write_sst3_flag) then begin
;
; Save the 'sst3' array for later processing.
;

saved_sst3_array = r_dataset_array;

i_dataset_name = 'sea_surface_temperature_3chan';
...
i_long_name =  'sea surface temperature (3 channel)';
i_standard_name = 'sea_surface_skin_temperature';
i_source = 'none';
i_comment = 'sea surface temperature from 3.7, 11, and 12 um channels (3 channels); non L2P core field; generally only nighttime SST';
...
endif; end if (ok_to_write_sst3_flag)
...
; MODIS_L2P_ERROR_STEP 19: If cannot read optional additional night variable 'bias_sst3' from night file, WARN sigevent thrown, keep going
...
if (ok_to_write_bias_sst3_flag) then begin
    r_bias_sst3 =  *(o_data_variable_structure.s_variable_array);
    PTR_FREE, o_data_variable_structure.s_variable_array;
    ...
    data_type_as_int = SIZE(r_bias_sst3,/TYPE);
    r_data_type = convert_int_type_to_char_type(data_type_as_int);
endif
endif else begin; end if (variable_exist_flag)
    if (debug_mode) then begin
...
; MODIS_L2P_ERROR_STEP 20: If cannot read optional additional night variable 'stdv_sst3' from night file, WARN sigevent thrown, keep going 
...
if (ok_to_write_stdv_sst3_flag) then begin
    r_stdv_sst3 =  *(o_data_variable_structure.s_variable_array);
    PTR_FREE, o_data_variable_structure.s_variable_array;
    ...
    data_type_as_int = SIZE(r_stdv_sst3,/TYPE);
    r_data_type = convert_int_type_to_char_type(data_type_as_int);
endif
endif else begin; end if (variable_exist_flag)
    if (debug_mode) then begin
...
; Read qual_sst3 from HDF file and write to NetCDF file.
;--------------------------------------------------------------------------------
...
; MODIS_L2P_ERROR_STEP 21: If cannot read optional additional night variable 'qual_sst3'from night file, WARN sigevent thrown, keep going
...
r_qual_sst3 =  *(o_data_variable_structure.s_variable_array);
PTR_FREE, o_data_variable_structure.s_variable_array;
r_status = find_netcdf_variable_attribute_info('scale_factor', o_data_variable_structure.s_attributes_array,r_slope);
r_status = find_netcdf_variable_attribute_info('add_offset',   o_data_variable_structure.s_attributes_array,r_intercept);
data_type_as_int = SIZE(r_qual_sst3,/TYPE);
r_data_type = convert_int_type_to_char_type(data_type_as_int);
endif else begin;
    if (debug_mode) then begin
        print, debug_module + 'Variable ' + i_variable_short_name + ' does not exist in ' + i_filename;
...
if ((ok_to_write_sst3_flag) AND (ok_to_write_bias_sst3_flag)) then begin
; Build an array to update the elements with.

; The calculation of the proximity confidence mapping depends on the stdv_sst3 variable being able to be read successfully.
; We only proceed if the variable exists.
if ((ok_to_write_qual_sst3_flag) AND (ok_to_write_stdv_sst3_flag)) then begin
r_updated_qual_sst3 = r_qual_sst3;
...
i_dataset_name = 'quality_level_3chan';
i_units = 'dimensionless';
r_fill_value = BYTE_FILL_VALUE;
i_valid_min = 0B;
i_valid_max = 5B;
i_coordinates = "lon lat";
i_long_name =  'quality level of 3 channel SST pixel';
i_comment =  '3 channel SST quality value; signed byte array:  WARNING Some applications are unable to properly handle signed byte values. If values are encountered > 127, please subtract 256 from this reported value';
...
endif; end if  (ok_to_write_qual_sst3_flag)
endif; end if ((ok_to_write_sst3_flag) AND (ok_to_write_bias_sst3_flag))
...
i_dataset_name = 'sses_bias_3chan';
standard_name  = 'sses_bias_3chan';
...
i_coordinates = "lon lat";
i_long_name =  'SSES (Sensor Specific Error Statistic) bias error for 3 channel SST';
i_source = 'none';
i_comment =  '3 channel SST bias error; signed byte array:  WARNING Some applications are unable to properly handle signed byte values. If values are encountered > 127, please subtract 256 from this reported value';

size_array = size(r_bias_sst3);
num_columns = size_array[1];
num_rows    = size_array[2];
r_bias_sst3_as_int = INTARR(num_columns,num_rows,/NOZERO);
...
if (GETENV('GHRSST_MODIS_L2P_CONVERTING_FROM_ORIGINAL_NETCDF_FILES') NE 'true') then begin
    l_status = convert_to_signed_byte(r_bias_sst3,r_intercept_bias_error,r_bias_sst3_as_int);
if (debug_mode) then begin
    print, debug_module + 'Writing ' + i_dataset_name;
endif
    r_status = write_gds2_variable(i_out_filename,$
...
print, 'ORIGINAL  : r_bias_sst3[0,0]           = ', r_bias_sst3[0,0];
print, 'CALCULATED: r_bias_sst3[0,0]*r_slope_bias_error + r_intercept_bias_error = ', r_bias_sst3[0,0] * r_slope_bias_error + r_intercept_bias_error;   
print, ''
...
if (ok_to_write_stdv_sst3_flag) then begin
i_dataset_name = 'sses_standard_deviation_3chan';
standard_name  = 'sses_standard_deviation_3chan';
if (i_convert_to_kelvin EQ 'yes') then begin
    i_units = 'kelvin';
end else begin
    i_units = 'celsius';
...
i_valid_max = SIGNED_BYTE_VALID_MAX; place holder
i_coordinates = 'lon lat';
i_long_name =  'SSES (Sensor Specific Error Statistic) standard deviation error for 3 channel SST';
i_source = 'none';
i_comment =  '3 channel SST standard deviation error; signed byte array:  WARNING Some applications are unable to properly handle signed byte values. If values are encountered > 127, please subtract 256 from this reported value';

; Because the variable r_bias_sst3 may not have been read successfully, we get the num_columns and num_rows from variable r_stdv_sst3
size_array = size(r_stdv_sst3);
num_columns = size_array[1];
num_rows    = size_array[2];
r_stdv_sst3_as_int = INTARR(num_columns,num_rows,/NOZERO);

; If we are not processing from original NetCDF file, use the "converted" intercept value. 
if (GETENV('GHRSST_MODIS_L2P_CONVERTING_FROM_ORIGINAL_NETCDF_FILES') NE 'true') then begin
    status = convert_to_signed_byte(r_stdv_sst3,r_intercept_standard_deviation_sst,r_stdv_sst3_as_int);
if (debug_mode) then begin
    print, debug_module + 'Writing ' + i_dataset_name;
endif
    r_status = write_gds2_variable(i_out_filename,$
...
print, 'ORIGINAL  : r_stdv_sst3[0,0]           = ', r_stdv_sst3[0,0];
print, 'CALCULATED: r_stdv_sst3[0,0]*r_slope_bias_error + r_intercept_bias_error = ', r_stdv_sst3[0,0] * r_slope_standard_deviation_sst + r_intercept_standard_deviation_sst;   
print, ''
print, 'Content of scaled byte variable to be written to NetCDF file:'
print, '          o_byte_scale_factor              = ', o_byte_scale_factor;
print, '          o_byte_add_offset                = ', o_byte_add_offset;   
...
endif; end if (ok_to_write_stdv_sst3_flag)
