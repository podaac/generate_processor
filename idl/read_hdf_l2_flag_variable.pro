...
;
; $Id: read_hdf_l2_flag_variable.pro,v 1.1 2006/11/06 19:55:37 qchau Exp $
; DO NOT EDIT THE LINE ABOVE - IT IS AUTOMATICALLY GENERATED BY CVS
; New Request #xxxx

FUNCTION read_hdf_l2_flag_variable,$
         i_filename,$
...
; Function read the l2_flag variable and some associated attributes from an HDF formatted file
; and return the array containing the data sets with 'r_' as the beginning of the name.
;
; Assumptions:
;
;   1. The file is opened already. 
;   2. The HDF variable has the associated attributes we are interested in. 
;   3. TBD.
;   4. TBD. 
;   5. TBD. 
;
...
if (sd_index EQ -1) then begin
    print, 'read_hdf_l2_flag_variable: ERROR, Cannot get index of HDF variable name: ' + i_variable_short_name + ' from file ' + i_filename;
    status = FAILURE;
...
if (units_index EQ -1) then begin
    print, 'read_hdf_l2_flag_variable: ERROR, Cannot get index of HDF attribute name: ' + l_unit_attribute_name + ' from file ' + i_filename;
    status = FAILURE;
    ; Must return immediately.
    return, status
endif
