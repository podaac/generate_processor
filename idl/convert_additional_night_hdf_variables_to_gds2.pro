...
FUNCTION convert_additional_night_hdf_variables_to_gds2,$
             i_filename,$
             i_out_filename,$
             i_num_lons,$
             i_num_lats,$
...
routine_name = "convert_additional_night_hdf_variables_to_gds2";
msg_type = "";
msg = ""
i_data = "";
...
debug_module = 'convert_additional_night_hdf_variables_to_gds2:';
debug_mode = 0
...
print, debug_module + 'i_filename ' + i_filename + ' i_variable_short_name ' + i_variable_short_name + ' variable_exist_flag ' + STRTRIM(STRING(variable_exist_flag),2);
endif
...
if (debug_mode) then begin
    print, debug_module + 'i_filename ' + i_filename + ' i_variable_short_name ' + i_variable_short_name + ' variable_exist_flag ' + STRTRIM(STRING(variable_exist_flag),2) + ' TEST_PARAMETER ' + TEST_PARAMETER + ' status ' + STRTRIM(STRING(status),2);
endif
...
if (debug_mode) then begin
    print, 'i_filename ' + i_filename + ' i_variable_short_name ' + i_variable_short_name + ' variable_exist_flag ' + STRTRIM(STRING(variable_exist_flag),2) + ' TEST_PARAMETER ' + TEST_PARAMETER + ' status ' + STRTRIM(STRING(status),2);
endif
...
if (TEST_PARAMETER EQ "BAD_QUAL_SST4_VARIABLE_READ") then begin
    ; Make variable r_qual_sst4 to see how it affects code below.
    if (N_ELEMENTS(r_qual_sst4)) then begin
...
flag_values[5] = 5B;
    ...
    dataset_name  = 'quality_level_4um';
...
endif; end if ((ok_to_write_qual_sst4_flag) AND (ok_to_write_stdv_sst4_flag)) 
endif; end if ((ok_to_write_sst4_flag) AND (ok_to_write_bias_sst4_flag)) then begin
...
print, 'Writing ' + i_dataset_name;
    ...
    write_status = write_gds2_variable(i_out_filename,$
                                   o_byte_variable_array,$
                                   i_dataset_name,$
                                   undefined_standard_name,$
                                   i_units,$
...
endelse
    ...
    if (TEST_PARAMETER EQ "BAD_BIAS4_VARIABLE_WRITE") then begin
        r_status = FAILURE;
    endif
...
; If we are not processing from original NetCDF file, use the "converted" intercept value. 
if (GETENV('GHRSST_MODIS_L2P_CONVERTING_FROM_ORIGINAL_NETCDF_FILES') NE 'true') then begin
    r_status = convert_to_signed_byte(r_stdv_sst4,r_intercept_standard_deviation_sst,r_stdv_sst4_as_int);
    ...
    dataset_name  = 'sses_standard_deviation_4um';
...
if (debug_mode) then begin
print, 'Content of original short variable from NetCDF file:'
print, '          r_slope_standard_deviation_sst = ', r_intercept_standard_deviation_sst;
