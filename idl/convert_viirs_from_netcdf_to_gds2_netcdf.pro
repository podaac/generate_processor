...
FUNCTION convert_viirs_from_netcdf_to_gds2_netcdf,$
             i_filename,$
             i_out_filename,$
...
; Function convert a VIIRS file from NetCDF4 format to NetCDF in GDS2 format.  This function is similiar to convert_modis_from_hdf_to_gds2_netcdf()
; except for converting from NetCDF into GDS2 format.
;
; Assumptions:
;
;------------------------------------------------------------------------------------------------
...
routine_name = "convert_viirs_from_netcdf_to_gds2_netcdf";
msg_type = "";
msg = ""
i_data = "";
...
debug_module = 'convert_viirs_from_netcdf_to_gds2_netcdf:';
debug_mode = 0
...
; Build a few array of strings to keep track of how long it take to read/write/convert each variable.
; Assume we are going to read/write/convert at most 20 strings.

variable_write_duration_array = STRARR(20);
variable_write_index = 0;
variable_read_duration_array = STRARR(20);
variable_read_index = 0;
variable_convert_duration_array = STRARR(20);
variable_convert_index = 0;
variables_read_duration_total = 0.0;
variables_write_duration_total = 0.0;
...
if (debug_mode) then begin
    print, debug_module + 'long_attributes_values = ', long_attributes_values;
endif
...
if (debug_mode) then begin
    print, debug_module + 'float_attributes_values = ', float_attributes_values;
endif
...
r_seconds_since_1981 = julday_to_seconds_since_1981($
...
dataset_name = "character_variables";
variable_read_start = SYSTIME(/SECONDS);
r_status = read_character_variables_from_netcdf_file($
               i_filename,$
               o_sensor_name,$
               o_start_node,$
               o_end_node,$
...
; We make a slight tweak to the name of the o_dsd_entry_id from VIIRS-JPL-L2P-v2016.0 to VIIRS_NPP-JPL-L2P-v2016.0
; to more accurately describe the sensor name.
; So we replace  VIIRS- with  VIIRS_NPP- in the o_dsd_entry_id variable.

o_dsd_entry_id = STRJOIN(STRSPLIT(o_dsd_entry_id, "VIIRS-", /REGEX, /EXTRACT, /PRESERVE_NULL), "VIIRS_NPP-")

; Replace VIIRS_NPP-JPL-L2P-v2016.0 with VIIRS_NPP-JPL-L2P-v2016.2
version_pos = STRPOS(o_dsd_entry_id,'v2');
if (version_pos GE 0) then begin
   @viirs_data_config.cfg
   o_dsd_entry_id = STRMID(o_dsd_entry_id,0,version_pos) + 'v' + const_gds2_product_version;
endif

r_status = create_viirs_gds2_cdf_file(i_out_filename,$
                                      i_start_time_array_element,$
                                      gds2_global_start,$
                                      gds2_global_stop,$
                                      ...
                                      o_platform,$
                                      o_comment,$
                                      o_start_node,o_end_node,o_day_or_night,$
                                      i_processing_type);
...
if (r_status NE SUCCESS) then begin
    ...
    ; If the output file exist, even though the status is FAILURE, we need to delete it.
    if FILE_TEST(i_out_filename) then begin
        print, debug_module + 'FILE_DELETE ' + i_out_filename;
        FILE_DELETE, i_out_filename, /QUIET;
    endif
...
instrument_function ="identify_bad_scan_lines";
...
if (debug_mode) then print, "instrument_function ",instrument_function," time_duration ",time_duration
...
instrument_function ="remove_dateline_discontinuity";
time_start = SYSTIME(/SECONDS);
if (run_slow_function) then begin 
...
instrument_function ="perform_spline_fit_on_controlled_points_with_improvements";
time_start = SYSTIME(/SECONDS);
l_status = perform_spline_fit_on_controlled_points_with_improvements( $
             r_num_bad_scan_lines,$
             r_scan_line_flag_array,$
             r_bad_longitude_fill_value,$
             r_bad_latitude_fill_value,$
...
;
    ...
    variable_write_start = SYSTIME(/SECONDS);
    r_status = write_gds2_variable(i_out_filename,$
                                   ...
                                   comment,$
                                   i_long_name);
    variable_write_end = SYSTIME(/SECONDS);
    variable_write_duration = variable_write_end - variable_write_start;
    variable_write_duration_array[variable_write_index++] = dataset_name + " " + STRING(variable_write_duration);
    variables_write_duration_total = variables_write_duration_total + variable_write_duration;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; Read arrays from NetCDF file and write to NetCDF file.
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
if (write_time_variable_flag EQ 1) then begin
    time_array = LON64ARR(1);
    time_array[0] = LONG(STRING(r_seconds_since_1981,FORMAT='(F0.3)'));   Convert 1140410521.000 to STRING, then back to LONG to preserve 1140410521
    long_name =  'reference time of sst file'; 
    standard_name = 'time' 
    dataset_name  = 'time';
    unit      = 'seconds since 1981-01-01 00:00:00';
    data_type = 'LONG';
    comment = 'time of first sensor observation';
...
; If the fill value is provided from NetCDF file, we attempt to use it
...
; If the valid_max value is provided from NetCDF file, we attempt to use it
...
;--------------------------------------------------------------------------------
; Read msec NetCDF file and write to NetCDF file.
;--------------------------------------------------------------------------------
...
;--------------------------------------------------------------------------------
; Read bias_sst from NetCDF file.
;--------------------------------------------------------------------------------
...
;--------------------------------------------------------------------------------
; Read stdv_sst from NetCDF file.
;--------------------------------------------------------------------------------
...
r_status = perform_proximity_confidence_mapping(num_lons, num_lats,$
    ...
    coordinates = "lon lat"
    comment = "thermal IR SST quality value; signed byte array: WARNING Some applications are unable to properly handle signed byte values. If values are encountered > 127, please subtract 256 from this reported value";
...
i_coordinates = "lon lat";
i_long_name =  'SSES (Sensor Specific Error Statistic) bias error';
i_source = ''; 
...
if (GETENV('GHRSST_MODIS_L2P_CONVERTING_FROM_ORIGINAL_NETCDF_FILES') NE 'true') then begin
    ...
    scaling_status = apply_scaling_from_short_to_byte($
                         ...
                         o_byte_add_offset);
if (debug_mode) then begin
print, debug_module + 'Content of original short variable from NetCDF file:'
print, debug_module + '          r_slope_bias_error          = ', r_slope_bias_error;
print, debug_module + '          r_intercept_bias_error      = ', r_intercept_bias_error;   
print, debug_module + 'ORIGINAL  : r_bias_sst[0,0]           = ', r_bias_sst[0,0];
print, debug_module + 'CALCULATED: r_bias_sst[0,0]*r_slope_bias_error + r_intercept_bias_error = ', r_bias_sst[0,0] * r_slope_bias_error + r_intercept_bias_error;   
print, debug_module + ''
print, debug_module + 'Content of scaled byte variable to be written to NetCDF file:'
print, debug_module + '          o_byte_scale_factor              = ', o_byte_scale_factor;
print, debug_module + '          o_byte_add_offset                = ', o_byte_add_offset;   
print, debug_module + 'ORIGINAL  o_byte_variable_array[0,0]) = ', o_byte_variable_array[0,0]
print, debug_module + 'CALCULATED:o_byte_variable_array[0,0]*o_byte_scale_factor + o_byte_add_offset     = ', o_byte_variable_array[0,0]*o_byte_scale_factor + o_byte_add_offset;
endif
...
i_valid_max = SIGNED_BYTE_VALID_MAX; place holder
i_coordinates = 'lon lat';
i_long_name =  'SSES (Sensor Specific Error Statistic) standard deviation error';
i_source = ''; 
...
if (debug_mode) then begin
print, debug_module +'Content of original short variable from NetCDF file:'
print, debug_module +'          r_slope_standard_deviation_sst = ', r_slope_standard_deviation_sst;
print, debug_module +'          r_intercept_standard_deviation_sst = ', r_intercept_standard_deviation_sst;
print, debug_module +'ORIGINAL  : r_stdv_sst[0,0]           = ', r_stdv_sst[0,0];
print, debug_module +'CALCULATED: r_stdv_sst[0,0]*r_slope_standard_deviation_sst + r_intercept_standard_deviation_sst = ', r_stdv_sst[0,0] * r_slope_standard_deviation_sst + r_intercept_standard_deviation_sst;
print, debug_module +''
print, debug_module +'Content of scaled byte variable to be written to NetCDF file:'
print, debug_module +'          o_byte_scale_factor              = ', o_byte_scale_factor;
print, debug_module +'          o_byte_add_offset                = ', o_byte_add_offset;
...
;print, 'convert_viirs_from_netcdf_to_gds2_netcdf: o_day_or_night = ', o_day_or_night; 

night_write_start = SYSTIME(/SECONDS);
l_status = convert_additional_night_viirs_netcdf_variables_to_gds2(i_filename,i_out_filename,num_lons,$
               num_lats,i_convert_to_kelvin,$
               r_num_bad_scan_lines,$
               r_scan_line_flag_array,$
               i_test_parameter) 
night_write_end  = SYSTIME(/SECONDS);
night_write_duration = night_write_end - night_write_start;


day_write_start = SYSTIME(/SECONDS);
l_status = convert_additional_day_viirs_netcdf_variables_to_gds2(i_filename,i_out_filename,$
               r_num_bad_scan_lines,$
               r_scan_line_flag_array,$
               i_test_parameter);
day_write_end  = SYSTIME(/SECONDS);
day_write_duration = day_write_end - day_write_start;
...
if (debug_mode) then begin
    ...
    print, debug_module + 'night_write_duration ', night_write_duration;
    print, debug_module + 'day_write_duration   ', day_write_duration;
    print, debug_module + 'variable_read_duration_array   ', variable_read_duration_array;
    print, debug_module + 'variables_read_duration_total  ', variables_read_duration_total;
    print, debug_module + 'variable_write_duration_array  ', variable_write_duration_array;
    print, debug_module + 'variables_write_duration_total ', variables_write_duration_total;
